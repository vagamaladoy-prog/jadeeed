...
`scripts/generate-atlas.mjs` генерирует узор в SVG: каждая нить основы окрашивается отдельно и сдвигается по вертикали, как у настоящего атласа — отсюда «рваные», чуть размытые края. Три варианта: `dense`, `light`, `strip`. Узор медленно «течёт» вниз (CSS-transform), на ПК слегка смещается за курсором, на телефоне — при скролле.
...
| `npm run assets` | пересобрать атлас, логотип, тестовые картинки |
